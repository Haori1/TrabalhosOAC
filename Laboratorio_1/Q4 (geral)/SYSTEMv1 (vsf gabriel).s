...
# Lembre-se: Os syscalls originais do Mars possuem precedencia sobre	#
...
### Obs.: a forma 'LABEL: instru��o' embora fique feio facilita o debug no Mars, por favor n�o reformatar
...
# Verifica o o numero da chamada do sistema
    ...
    addi    t0, zero, 134		# ecall 41 = print hex
    beq     t0, a7, goToPrintHex
    ...
    addi    t0, zero, 49              # ecall 49 = SD Card read
    beq     t0, a7, goToSDread
    addi    t0, zero, 149              # ecall 49 = SD Card read
    beq     t0, a7, goToSDread
...
goToExitDE2:	j       goToExit    ########### ecall 10 ou 110
...
printChar:	 li t4, 0xFF			# t4 temporario
		slli 	t4, t4, 8			# t4 = 0x0000FF00 (no RARS, nao podemos fazer diretamente "addi rd, rs1, 0xFFF")
...
loopReadCharKDMMIO:  	lw     	a7, 0(t0)   			# le o bit de flag do teclado
			andi 	a7, a7, 0x0001		# masacara bit 0
			beq     a7, zero, loopReadCharKDMMIO  # testa se uma tecla foi pressionada
    			lw 	a7, 4(t0)			# le o ascii da tecla pressionada
...
loopReadCharKDMMIODE2: 	lw     	a7, 0(t0)   			# le o bit de flag do teclado
			andi 	a7, a7, 0x0001		# masacara bit 0
			beq     a7, zero, loopReadCharKDMMIODE2  # testa se uma tecla foi pressionada
    			lw 	a7, 4(t0)			# le o ascii da tecla pressionada
			j fimreadChar				# fim Read Char
...
naoehshiftChar:	   	add     t3, fp, t3                   	# endereco na tabela de scancode da tecla com ou sem shift
    	lb      a7, 0(t3)				# le o ascii do caracter para a7
    	beq     a7, zero, atualizaBufferChar		# se for caractere nao imprimivel volta a ler
...
loopreadString: beq 	a1, v1, fimreadString   	# buffer cheio fim
		mv 	t9, ra			# salva ra
		jal 	readChar			# le um caracter do teclado (retorno em a7)
		mv 	ra, t9			# recupera ra
		beq 	a7, 0x0A, fimreadString	# se for tecla ENTER fim
...
fimreadString: 	sb 	zero, 0(a0)			# grava NULL no buffer
		addi 	a7, a0, -1			# Para que a7 tenha o endereco do ultimo caractere digitado
		lw 	ra, 0(sp)			# recupera ra
		addi 	sp, sp, 4			# libera espaco
		jr 	ra				# retorna
...
ehnegReadInt:	sub a7,zero,a7		# se for negativo
...
printFloat:	addi 	sp, sp, -4
		...
		li 	t0, '+'			# define sinal '+'
		fcvt.w.s 	s1, f12			# recupera o numero float
		li		t0, 0x8
...
ehposprintFloat: sb 	t0, 0(s0)			# coloca sinal no buffer
		...
		# Encontra o expoente em t0
		 fcvt.w.s 	t0, f12			# recupera o numero float
		 lui	t1, 0x7F800
		...
		fcvt.w.s 	t1, f12			# recupera o numero float
		lui 	t2, 0x7FFFF
		srli	t2, t2, 8
...
cont2printFloat:	fmv.s 	f4, f0			# inicia com o numero x 
		 	mtc1 	zero, f3			# contador come�a em 0
loop2printFloat:  	flt.s 	t3, f4, f10			# resultado eh < que 10? entao fim
		...
		# Neste ponto tem-se em t4 se f0<1, em f3 o expoente de 10 e f0 0 m�dulo do numero e s1 o sinal
		# e em f4 um n�mero entre 1 e 10 que multiplicado por Ef3 deve voltar ao numero		
...
expposprintFloat: 	sb 	t0, 0(s0)				# coloca no buffer
			...
			addi 	t0, t0, 48			# converte para ascii
			sb 	t0, 0(s0)			# coloca no buffer
			fmv.s 	t0, t2				# move o resto pra t0
			addi 	t0, t0, 48			# converte para ascii
...
readFloat: addi sp, sp, -4			# aloca espaco
	...
	jal	readString			# le string, retorna a7 ultimo endereco e v1 numero de caracteres
	mv 	s0, a7			# ultimo endereco da string (antes do \0)
	mv 	s1, v1			# numero de caracteres digitados
...
sdRead:		DE2(sdReadDE2)
		...
		li 	a7, 13		
		ecall		
		mv 	t3, a7		# Salva o Descritor
		mv 	a0, t3		# Define o descritor
		la 	a1, TempBuffer		# Define o Endereco de Buffer do cartao SD
		li 	a2, 512		# numero de bytes do buffer
		mv 	t4, t0		# endereco origem = num bytes a pular
	
LoopSD: 	ble 	t4, zero, ForaSD	# leu todos os setores ?
		li 	a7, 14			# Le um bloco de 512 bytes
		ecall			
		addi 	t4, t4, -512		# subtrai o endereco de origem
		j 	LoopSD			# volta ao loop
	
ForaSD: 	sub  	a2, zero, t4		# numero de bytes faltantes
		li	a7, 14		# le o numero de bytes faltantes a pular
		ecall		

		mv 	a0, t3		# le os bytes desejados
		mv 	a1, t1
		mv 	a2, t2
		li 	a7, 14
		ecall		
		blt 	a7, zero, FimSD	# se a7 <0 entao deu erro

		mv 	a0, t3		# recupera o descritor
		li 	a7, 16			# fecha o arquivo
		ecall		
		li 	a7, 0			# retorna sem erro
		j 	FimSD			# retorna
...
sdDataReady:	lw      t2, 0(s2)                     # Le word do buffer
 ...
 FimSDRead:	li      a7, 0                          # Sucesso na transferencia.         NOTE: Hardcoded. Um teste de falha deve ser implementado.
...
############################################
time: 	DE2(timeDE2)
	li 	a7,30				# Chama o ecall do Mars
	ecall
	j 	fimTime				# saida
...
############################################
sleep: DE2(sleepDE2)
	li 	a7, 32				# Chama o ecall do Mars
	ecall			
	j 	fimSleep			# Saida
...
random: DE2(randomDE2)		
	li 	a7,41			# Chama o ecall do Mars
	ecall	
	j 	fimRandom		# saida
