...
#    beq     t0, a7, goToPopEvent		# precisa implementar?
...
#	lw      x11,   40(sp)	# a1 as vezes usado como retorno de valor
    lw      x12,   44(sp)
    lw      x13,   48(sp)
    lw      x14,   52(sp)
    lw      x15,   56(sp)
...
loopreadString: beq 	a1, a3, fimreadString   	# buffer cheio fim
		...
		beq 	t6, 0x0A, fimreadString	# se for tecla ENTER fim
...
printFloat:	addi 	sp, sp, -4
		sw 	ra, 0(sp)				# salva ra
		frrm s2						# salva o modo de arrendondamento das instruçoes FP atual
...
#		# Faz a leitura do arquivo imagem (raw) do cartao SD: "SD.bin" 
